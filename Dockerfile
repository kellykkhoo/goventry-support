...
# Install dependencies only if package.json exists. Falls back to
# `--legacy-peer-deps` when npm install fails — many user uploads
# (Figma "make" exports, Replit projects, older React templates) have
# peer-dep conflicts that npm 7+ refuses to auto-resolve. The fallback
...
RUN if [ "$(cat .project-type)" = "static-html" ]; then \
      ...
      if [ -d .next/standalone ]; then \
        ...
        # Some Vite/Rollup configs emit to dist-vite/, dist-rollup/, etc.
        # Rename the first match to dist/ so the static-dist serve path picks it up.
